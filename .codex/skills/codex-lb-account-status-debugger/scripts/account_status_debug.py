...
from codex_lb_shared.utils import (  # noqa: E402, I001
    format_epoch_and_iso as _format_epoch_and_iso,
    iso_utc_from_epoch_seconds as _iso_utc_from_epoch_seconds,
...
def _default_codex_lb_dir() -> Path:
...
def main() -> int:
    ...
    parser.add_argument(
        "--codex-lb-dir",
        default=str(_default_codex_lb_dir()),
        help="Base codex-lb data dir (default: %(default)s).",
    )
    parser.add_argument("--accounts-db", default="", help="Override accounts.db path.")
    parser.add_argument("--store-db", default="", help="Override store.db path.")
    ...
    if bool(args.apply) and bool(args.apply_dry_run):
        print("Pass only one of --apply or --apply-dry-run.", file=sys.stderr)
        return 2

    apply_like = bool(args.apply) or bool(args.apply_dry_run)

    codex_lb_dir = Path(str(args.codex_lb_dir)).expanduser()
    store_db_path = Path(str(args.store_db)).expanduser() if args.store_db else (codex_lb_dir / "store.db")
    accounts_db_path = Path(str(args.accounts_db)).expanduser() if args.accounts_db else (codex_lb_dir / "accounts.db")
    ...
    if args.message_draft and len(accounts) != 1:
        ...
        return 2
    if apply_like and len(accounts) != 1:
        print("--apply/--apply-dry-run only supports a single account target.", file=sys.stderr)
        return 2
    if apply_like and not bool(args.probe):
        print(
            "--apply/--apply-dry-run requires --probe so status is based on a fresh upstream result.",
            file=sys.stderr,
        )
        return 2
    ...
    if apply_like:
        ...
        if report.probe_status_code is None:
            print("--apply/--apply-dry-run requires --probe to produce a probe result.", file=sys.stderr)
            return 2
        ...
        if int(report.probe_status_code) == 200:
            ...
            if probe_code == "rate_limit_exceeded":
                ...
                print(
                    f"Unsupported probe error_code for --apply/--apply-dry-run: {report.probe_error_code!r} (HTTP 429)",
                    file=sys.stderr,
                )
                return 2
        else:
            print(
                f"Unsupported probe status_code for --apply/--apply-dry-run: {report.probe_status_code} "
